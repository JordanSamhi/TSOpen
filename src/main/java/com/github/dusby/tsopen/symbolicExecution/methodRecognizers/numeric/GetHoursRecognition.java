...
public class GetHoursRecognition extends NumericMethodsRecognitionHandler {
	...
	public boolean processNumericMethod(SootMethod method, Value base, SymbolicValue sv) {
		...
		if(declaringClass.getName().equals(Constants.JAVA_UTIL_DATE) && methodName.equals(Constants.GET_HOURS)) {
			if(Utils.containsTag(base, Constants.NOW_TAG, this.se)) {
				sv.addTag(new StringConstantValueTag(Constants.HOUR_TAG));
				return true;
			}
		}
		return false;
